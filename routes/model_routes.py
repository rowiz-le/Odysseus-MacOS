...
except ImportError:  # test stubs may expose only the endpoint model
    DbSession = None
...
def _httpx_get(url: str, **kwargs):
    try:
        ...
        if "verify" in kwargs and "verify" in str(e):
            retry_kwargs = dict(kwargs)
            retry_kwargs.pop("verify", None)
            return httpx.get(url, **retry_kwargs)
        raise


def _safe_models_url(base: str) -> str:
    try:
...
def _probe_endpoint(base_url: str, api_key: str = None, timeout: int = 5) -> List[str]:
    ...
    if _detect_provider(base) == "anthropic":
        ...
        try:
            r = _httpx_get(url, headers=headers, timeout=timeout, verify=llm_verify())
            r.raise_for_status()
            data = r.json()
...
def _ping_endpoint(base_url: str, api_key: str = None, timeout: float = 1.5) -> Dict[str, Any]:
    ...
    try:
        r = _httpx_get(base, headers=headers, timeout=timeout, verify=llm_verify())
